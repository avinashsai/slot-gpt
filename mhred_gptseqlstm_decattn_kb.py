...
vocab[3] = "<|endoftext|>"
reverse_vocab = {vocab[key]:i for i, key in enumerate(vocab)}
print("Vocabulary Size: {} ".format(len(vocab)))

tokenizer = AutoTokenizer.from_pretrained(gpt_path)
gpt_model = AutoModelWithLMHead.from_pretrained(gpt_path)
gpt_model = gpt_model.to(device)
word_embeddings = gpt_model.transformer.wte.weight
...
embedding_matrix = torch.zeros(len(vocab), tgt_emb_dim)
for i, word in enumerate(reverse_vocab):
    text_index = tokenizer.encode(word, add_prefix_space=True)
    embedding_matrix[i, :] = gpt_model.transformer.wte.weight[text_index, :][0]

kb_vocab["<|endoftext|>"] = kb_vocab['<end>']
del kb_vocab['<end>']
celeb_vocab["<|endoftext|>"] = celeb_vocab['<end>']
del celeb_vocab['<end>']

reverse_kb_vocab = {kb_vocab[key]:key for key in kb_vocab}
reverse_kb_vocab = dict(sorted(reverse_kb_vocab.items()))
reverse_celeb_vocab = {celeb_vocab[key]:key for key in celeb_vocab}
reverse_celeb_vocab = dict(sorted(reverse_celeb_vocab.items()))

kb_embedding_matrix = torch.zeros(len(kb_vocab), tgt_emb_dim)
for i in range(len(reverse_kb_vocab)):
    text_index = tokenizer.encode(reverse_kb_vocab[i], add_prefix_space=True)
    kb_embedding_matrix[i, :] = gpt_model.transformer.wte.weight[text_index, :][0]

celeb_embedding_matrix = torch.zeros(len(celeb_vocab), tgt_emb_dim)
for i in range(len(reverse_celeb_vocab)):
    text_index = tokenizer.encode(reverse_celeb_vocab[i], add_prefix_space=True)
    celeb_embedding_matrix[i, :] = gpt_model.transformer.wte.weight[text_index, :][0]

class MultimodalHRED(nn.Module):
    def __init__(self, src_vocab_size, tgt_vocab_size, pretrained_embed, src_emb_dim, tgt_emb_dim, 
        ...
        self.embedmatrix = pretrained_embed
        ...
        if self.use_kb:
            self.kb_embedmatrix = kb_embedding_matrix
            self.celeb_embedmatrix = celeb_embedding_matrix
            self.kb_encoder = KbEncoder(self.kb_embedmatrix, self.kb_emb_size, self.kb_hidden_size,
                                rnn_type='GRU', num_layers=1, batch_first=True,
                                dropout=0, bidirectional=False)

            self.celeb_encoder = KbEncoder(self.celeb_embedmatrix, self.kb_emb_size, self.kb_hidden_size,
                                rnn_type='GRU', num_layers=1, batch_first=True,
                                dropout=0, bidirectional=False)

        self.encoder = EncoderRNN(self.embedmatrix, self.src_emb_dim, self.enc_hidden_size, 
    ...
    def forward(self, text_enc_input, image_enc_input, text_enc_in_len=None, dec_text_input=None,
                ...
                beam_size=1, kb_vec=None, celeb_vec=None, kb_len=None, celeb_len=None, 
                epoch=None, istest=False):
        ...
        for turn in range(0, context_size):
            ...
            if(turn == 0):
                if(istest is False):
                    c1_att_wgts_c1, c1_enc_rep_c1 = attention(encoder_outputs, encoder_outputs, encoder_outputs, 
                                                          dropout=attn_drop)
                else:
                    c1_att_wgts_c1, c1_enc_rep_c1 = attention(encoder_outputs, encoder_outputs, encoder_outputs)
                ...
                if(istest is False):
                    c2_att_wgts_c2, c2_enc_rep_c2 = attention(encoder_outputs, encoder_outputs, encoder_outputs, 
                                                          dropout=attn_drop)
                else:
                    c2_att_wgts_c2, c2_enc_rep_c2 = attention(encoder_outputs, encoder_outputs, encoder_outputs)
        ...
        if use_kb:
            ...
            if(istest is False):
                _, kb_out = attention(kb_out, kb_out, kb_out, dropout=kb_attndrop)
            else:
                _, kb_out = attention(kb_out, kb_out, kb_out)
            ...
            if(istest is False):
                _, celeb_out = attention(celeb_out, celeb_out, celeb_out, dropout=kb_attndrop)
            else:
                _, celeb_out = attention(celeb_out, celeb_out, celeb_out)
...
for batch_id in range(num_test_batch):
    ...
    dec_output_prob = model(text_enc_input, image_enc_input, text_enc_in_len, 
                            ...
                            celeb_len=celeb_len, istest=True)
